...
> {
   ...
   * block style, list styles without an associated Solid component.
   *
 ...
 */
export interface PortableTextSolidComponents {
  /**
   * Object of Solid components that renders different types of objects that might appear
   ...
   *
   * The object has the shape `{styleName: SolidComponent}`, where `styleName` is the value set
   * in individual `style` attributes on blocks.
   *
   * Can also be set to a single Solid component, which would handle block styles of _any_ type.
   */
  block:
   ...
   *
   * Can also be set to a single Solid component, which would handle lists of _any_ type.
   */
  list:
   ...
   *
   * The object has the shape `{listItemType: SolidComponent}`, where `listItemType` is the value
